...
class TreinoController {
    ...
    duplicarTreinoParaAlunos = async (req: Request, res: Response) => {
        try {
            const userId = (req as any).user?.id as string;
            const resposta = await this.service.duplicarTreinoParaAlunos(
                req.params.id as string,
                req.body,
                userId,
            );
            ...
            const msg = error instanceof Error ? error.message : 'Erro desconhecido';
            console.error('[TreinoController] [duplicarTreinoParaAlunos] Erro interno:', msg);
            return CommonResponse.serverError(
                res,
                { message: msg },
                HttpStatusCode.INTERNAL_SERVER_ERROR.message,
    ...
    deleteTreino = async (req: Request, res: Response) => {
        try {
            const userId = (req as any).user?.id as string;
            const resposta = await this.service.deleteTreino(
                req.params.id as string,
                req.query,
                userId,
            );
            ...
            const msg = error instanceof Error ? error.message : 'Erro desconhecido';
            console.error('[TreinoController] [deleteTreino] Erro interno:', msg);
            return CommonResponse.serverError(
                res,
                { message: msg },
                HttpStatusCode.INTERNAL_SERVER_ERROR.message,
