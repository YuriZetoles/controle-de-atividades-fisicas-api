...
sessaoRegistry.registerPath({
    ...
    description: `Retorna a sessão \`EM_ANDAMENTO\` do aluno autenticado, ou \`data: null\` se não houver nenhuma.
...
- \`duracao_minutos\`: null se sessão ainda \`EM_ANDAMENTO\` e sem \`fim\` registrado
- \`volume_total_kg\`: soma de \`carga_utilizada × repeticoes_realizadas\` das séries \`CONCLUIDA\`
- \`taxa_conclusao\`: percentual de séries concluídas (0–100)`,
    tags: ["Sessao"],
    security: [{ BearerAuth: [] }],
    request: {
        params: z.object({
